...
class XigSession(AbstractSession):
    ...
    def __init__(self, xig_core, url, xbee_addr):
        ...
        self.__always_send_help = getattr(self.__core.getConfig(), "global_always_send_help", False)

        if url in ('@PING', '@ping'):
            self.appendSessionToXBeeBuffer("OK\r\n")
        if url in ('@HELP'):
        ...
        # not a good idea to allow anyone on the pan to terminate the process
